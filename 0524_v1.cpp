...
int main(void) { 
	...
	flag = readRecords( cSet, fileName, rTime ); 	// call: read records from a file
	if(flag){
...
bool readRecords(vector<collegeType> &cSet, string & fileName, float & rTime ) {   // read records from a file  
	...
	string coName, depName, stLevel;
 	...
 	if ( !inFile.is_open() ){ 	// amy : add "!"						// unable to open file
 		cout << endl << "### " << fileName << " does not exist! ###" << endl;
		return false;  // amy : add "return false"
	 }	
 	else {
 	...
 	// timer = clock();						// start the timer  // amy : don't need the line
...
void displayAll(vector<collegeType> &cSet) { 	// output all on screen
 	...
 	for (vector<collegeType>::iterator it = cSet.begin(); it < cSet.end(); ++it) {
  		cout << "(" << setw(2) << ++j << ") " << setw(10) << it->cname << setw(30)
			 << it->dname;
 		cout << setw(8) << it->level << setw(6) << it->total << " students" << endl;   // amy : "<<" setw(6)
 	} // end for
} // end displayAll
...
void WriteTimeRecords( string fileName, string fname, float readTime, float runTime, float writeTime ) {  // amy : "float write time"
 	int j = 0;
 	fstream outFile; 							// output file handle
				...
				if ( tempS[walk].cname != tempS[head].cname ) {					
					...
					if ( show2 == '-' ) {	// �ƧǤ覡�� ����
						...
						for ( int i = head; i < walk ; i++ )
							tempS[i] = temp[walk + head - i - 1]; // amy : add ";"
					}	// if
