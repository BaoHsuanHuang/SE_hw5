...
bool readRecords(vector<collegeType> &cSet, string & fileName, float & rTime ) {   // read records from a file
	...
	string coName, depName, stLevel;
 	...
 	if (!inFile.is_open()) 							// unable to open file  // amy : add "!"
...
// 	timer = clock();						// start the timer  // amy : delete this line
	...
	fileName = temp;
 	if ( !cSet.size() ){
 		cout << endl << "### Get nothing from the file " << fileName << " ! ### " << endl; return false;} return true;  // amy : add "return false" & "return true"
} // end readRecords
...
void displayAll(vector<collegeType> &cSet) { 	// output all on screen
 	...
 	for (vector<collegeType>::iterator it = cSet.begin(); it < cSet.end(); ++it) {
  		cout << "(" << setw(2) << ++j << ") " << setw(10) << it->cname << setw(30)
			 << it->dname;
 		cout << setw(8) << it->level << setw(6) << it->total << " students" << endl;  // amy : add "<<" setw(6)
 	} // end for
} // end displayAll
...
void WriteTimeRecords( string fileName, string fname, float readTime, float runTime, float writeTime) { // amy : "float writeTime"
 	int j = 0;
 	fstream outFile; 							// output file handle
